...
class AccountHttp extends Http_1.Http {
    ...
    constructor(url, networkHttp) {
        ...
        this.accountRoutesApi = new nem2_library_1.AccountRoutesApi(this.apiClient);
    }
    /**
    ...
    getAccountProperties(address) {
        return rxjs_1.from(this.accountRoutesApi.getAccountProperties(address.plain())).pipe(operators_1.map((accountProperties) => {
    ...
    getAccountsInfo(addresses) {
        ...
        return rxjs_1.from(this.accountRoutesApi.getAccountsInfo(accountIdsBody)).pipe(operators_1.map((accountsInfoMetaDataDTO) => {
            return accountsInfoMetaDataDTO.map((accountInfoDTO) => {
                return new AccountInfo_1.AccountInfo(accountInfoDTO.meta, Address_1.Address.createFromEncoded(accountInfoDTO.account.address), new UInt64_1.UInt64(accountInfoDTO.account.addressHeight), accountInfoDTO.account.publicKey, new UInt64_1.UInt64(accountInfoDTO.account.publicKeyHeight), accountInfoDTO.account.mosaics.map((mosaicDTO) => new Mosaic_1.Mosaic(mosaicDTO.id, mosaicDTO.amount)), new UInt64_1.UInt64(accountInfoDTO.account.importance), new UInt64_1.UInt64(accountInfoDTO.account.importanceHeight));
            });
        }));
    }
        ...
        return rxjs_1.from(this.accountRoutesApi.getAccountsNames(accountIdsBody)).pipe(operators_1.map((accountNames) => {
            return accountNames.map((accountName) => {
                return new AccountNames_1.AccountNames(Address_1.Address.createFromEncoded(accountName.address), accountName.names.map((name) => {
                    new NamespaceName_1.NamespaceName(new NamespaceId_1.NamespaceId(name), name);
                }));
            });
        }));
    }
